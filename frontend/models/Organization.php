...
class Organization extends \yii\db\ActiveRecord
{
    ...
    {
        return [
            [['parent_id', 'type_id', 'rating', 'created_at', 'updated_at', 'created_by', 'updated_by'], 'integer'],
    ...
    {
        return $this->hasMany(OrganizationServiceOffer::className(), ['organization_id' => 'id']);
    }
     ...
     * @return \yii\db\ActiveQuery
     */
    public function getOrganizationServiceOffersList()
    {
      $offers = $this->organizationServiceOffers;
      $service_offers = array();
      foreach ($offers as $offer) {
        $service_offers[] = ['label' => $offer->serviceOffer->service_offer, 'icon' => $offer->serviceOffer->icon];
      }
   ...
   public static function getOrganizationList()
   {
    ...
    public static function getUserOrganizationID()
    {
        $user_id = Yii::$app->user->id;
        $organization_id = Organization::find()->where(['created_by' => $user_id])->select('id')->one();
