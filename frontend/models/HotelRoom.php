...
//    }
    ...
    public static function getAvailableHotelRoomList($daterange)
    {
          ...
          // where start_date is null
           ...
           // select organization_id, room_type_id, COUNT(*) as num from `hotel_room` left join `product` on hotel_room.id = product.hotel_room_id and product_status not in ('prospecting') GROUP BY organization_id, room_type_id
          $sql = "

             select hotel_room.id as hotel_room_id, organization.id, organization.name,organization.rating,hotel_room.cost as room_cost,organization.picture, COUNT(*) as num, hotel_room_type.type from `hotel_room` left join `product` on hotel_room.id = product.hotel_room_id and product_status not in ('prospecting') left join `organization` on hotel_room.organization_id = organization.id left join `hotel_room_type` on hotel_room.room_type_id = hotel_room_type.id GROUP BY organization_id, room_type_id
